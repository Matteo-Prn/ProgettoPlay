...
/**
 * Controller per la barra laterale dell'amministratore.
 * Questa classe gestisce le azioni dei pulsanti nella barra laterale,
 */
public class SidebarAdminController implements Initializable {
    ...
    @FXML
    private Label sectionSidebarLabel;
    ...
    private static final String SECTION_SIDEBAR_LABEL = "sectionSidebarLabel";
    ...
    private void handleCreate(Event event) {
        final String PATH = "CreateExercise.fxml";
        SceneLoader.loadSceneAndGetController(PATH, createButton);
    }

    private void handleRanking(Event event) {
        final String PATH = "RankAdminView.fxml";
        SceneLoader.loadSceneAndGetController(PATH, rankingButton);
    }

    private void handleStatics(Event event) {
        final String PATH = "StatisticsAdmin.fxml";
    ...
    private void updateText(Language lang) {
        ...
        createButton.setText(I18NUtils.getUI(SUBFOLDER, CREATE, lang.getLocale()));
        languageSection.setText(I18NUtils.getUI(SUBFOLDER, LANGUAGE_SECTION, lang.getLocale()));
        sectionSidebarLabel.setText(I18NUtils.getUI(SUBFOLDER, SECTION_SIDEBAR_LABEL, lang.getLocale()));
    ...
    private void updateSelectedButton(MFXButton selectedButton) {
        ...
        createButton.getStyleClass().remove("selected-button-variant");

        setUpButtons();

        if (selectedButton != null) {
            if (selectedButton == createButton) {
                selectedButton.getStyleClass().add("selected-button-variant");
            } else {
    ...
    public void selectCreateButton() {
        updateSelectedButton(createButton);
    }
